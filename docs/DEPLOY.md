...
health-check, регіон Frankfurt, `AI_MAX_CONCURRENCY=4` і `MAX_UPLOAD_MB=80` під
безкоштовний тариф, `DATA_DIR=./data`, `DATABASE_URL=sqlite:///./estimator.db`.

> **Диска немає — і це навмисно.** Render відхиляє Blueprint із диском на
> безкоштовному тарифі. Тому сховище тимчасове: проєкти й завантажені PDF
> зникають при кожному redeploy, рестарті та пробудженні після сну.
> Каталог і правила їдуть разом із кодом і перезавантажуються самі, тож застосунок
> завжди піднімається робочим — але **готовий кошторис треба експортувати в XLSX
> одразу**, не залишаючи «на завтра».
>
> Щоб дані зберігалися: тариф **Starter** ($7/міс) + повернути в `render.yaml`
> блок диска й `DATA_DIR=/var/data`, або підключити безкоштовний Postgres
> (Render → New → Postgres) і вписати його URL у `DATABASE_URL`.
...
| **Немає постійного сховища** | На безкоштовному тарифі Render не дозволяє диски (`disks are not supported for free tier services`), тому `render.yaml` його не оголошує. БД і завантажені PDF зникають при кожному redeploy, рестарті та пробудженні після сну. |
...
> Render, достатньо оновити сид і зробити push: на безкоштовному тарифі база
> й так створюється заново при кожному redeploy.
